...
| `delete-existing-release` | false                            | Flag indicating whether to delete then re-create a release if it already exists. This will not delete its associated tag. Nor will it update the release to use the new tag's SHA. If you want to update the associated tag with the new sha, use the [im-open/create-tags](https://github.com/im-enrollment/create-tags-action) action.<br/>Accepted Values: `true\|false`.  Default: `false`.    |
